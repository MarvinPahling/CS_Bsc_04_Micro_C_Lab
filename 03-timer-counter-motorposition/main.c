...
#define MA1 1      //PerA=PWM1  PerB=TioB0   <-- Pos=Gpio-IRQ							 //Encoder Input Motor 0?

#define MB0 26     //PerA=DCD1  PerB=TioA2   <-- Pos=Timer-IRQ  Geschwindigkeit=Timer2   
#define MB1 9      //PerA=DRxD  PerB=NPCs1   <-- Pos=Gpio-IRQ							 //Encoder Input Motor 1?

#define MC0 0      //PerA=PWM0  PerB=TioA0   <-- Pos=Timer-IRQ  Geschwindigkeit=Timer0   
#define MC1 8      //PerA=CTS0  PerB=ADTrg   <-- Pos=Gpio-IRQ							 //Encoder Input Motor 2?

typedef enum __attribute__((packed)) {TV_MANUELL,TV_VARIABEL,TV_END} TV_MODE;

typedef enum __attribute__((packed)) {POSITION_MANUELL,POSITION_KONTINUIERLICH,POSITION_END} POSITION_MODE;
...
//  uint32_t i_state = interrupts_get_and_disable();
	...
	uint32_t pinChanges  = *AT91C_PIOA_ISR;		// IRQ-Quelle auslesen + IRQ zurücksetzen
...
//	aic_mask_off(AT91C_ID_PIOA);
	aic_set_vector(AT91C_ID_PIOA, AT91C_AIC_SRCTYPE_INT_HIGH_LEVEL | // Art 
