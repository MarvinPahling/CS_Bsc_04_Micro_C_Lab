...
static aic_sys_vl_t nxt_avr_aic_sys_vl;
...
static enum { //Link States
	...
	LS_RETRY,
	LS_RESET 
} __attribute__((packed)) link_state = LS_CLOSED;
 ...
 * to go.
 */
static void nxt_avr_link_init(void)
{
 ...
 */
static void nxt_avr_1kHz_update(void)
{
  int state;
  switch(link_state)
  {
  ...
  case LS_RETRY:
    ...
    if (state == 0) {
      ...
      if (state < 0) {
        STATS(nxt_avr_stats.not_ok++);
        link_state = LS_RESET;
      }
      else {
        STATS(nxt_avr_stats.still_busy++);
...
void nxt_avr_init(void)
{
  ...
  aic_sys_register_pit(&nxt_avr_aic_sys_vl,nxt_avr_1kHz_update);
}
...
button_bf_t buttons_get(void)
{
	...
	return *((button_bf_t *)&button_state);
