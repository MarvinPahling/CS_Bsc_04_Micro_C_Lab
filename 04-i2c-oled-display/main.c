...
void pwm_isr_entry(void)
{
	...
	if(pinChanges & (1<<0)) {
		if(scope.mode == SCOPE_CONTINUE) {
			...
			scope.buf0[SCOPE_SIZE-1]=pio & i2c_mask[NXT_I2C_PORT].i2c_sda ? 100:0;
			scope.buf1[SCOPE_SIZE-1]=pio & i2c_mask[NXT_I2C_PORT].i2c_scl ? 250:150;
		}
...
int scope_init(SCOPE_MODE mode,uint32_t zyklus_us)
{
	...
	if(mode != SCOPE_STOP) {
		if((zyklus_us < 100) || (zyklus_us > 10900)) {  //1/48.000.000*8=10,9ms
			scope.mode=SCOPE_STOP;
			return -1;
		}
		
		AT91C_BASE_PWMC->PWMC_CH[0].PWMC_CMR=3 /* =/8 | AT91C_PWMC_CALG | AT91C_PWMC_CPOL | AT91C_PWMC_CPD*/;
...
// i2c_write_byte()
//   Zum Senden eines Bytes incl. Ack-Bit (optional mit Start- und Stop-Bedingung)
// i2c_read_byte()
...
#define MCP2317_0_DEFVALB  0x07   // --> If the Associated pin level is the opposite, an interrupt occurs
#define MCP2317_0_INTCONA  0x08   //Interrupt Control (Default: 0)
#define MCP2317_0_INTCONB  0x09   // --> 0->Both Edge triggered 1->Opposite to DEFVAL
#define MCP2317_0_GPPUA    0x0C   //Pull Up Register (Default: 0)
#define MCP2317_0_GPPUB    0x0D   // --> 0->PullUp Enabled         1-> PullUp Disabled
#define MCP2317_0_INTFA    0x0E   //Interrupt Flag Register (Default: 0)
#define MCP2317_0_INTFB    0x0F   // --> 0->Interrupt not pending  1-> Interrupt pending
#define MCP2317_0_INTCAPA  0x10   //Interrupt Capture Register
#define MCP2317_0_INTCAPB  0x11   // --> These bits reflects the logic level at the time of interrupt
...
#define MCP2317_0_OLATB    0x15   // --> Reading reads the OLAT (not the port) Write modiesfies die OLAT
...
#define MCP2317_IOCON_MIRROR 0x40  //0-> InTA/IntB separat     1->INTA+INTB Connected
#define MCP2317_IOCON_SEQOP  0x20  //0-> Sequential enabled    1-> Sequential Disabled (Byte-Mode)
#define MCP2317_IOCON_DISSLW 0x10  //0-> SlewRate SDA Enabled  1->SlewRate disabled
#define MCP2317_IOCON_HAEN   0x08  //0->Disable AdrressPins    1->Enable AdressPins (nur SPI)
#define MCP2317_IOCON_ODR    0x04  //0->Int Active Driven      1->Int Pin Open Drain
...
//Zuordnung IO-Expander Port/Bit zur Joystick-Multiplexer (wird hier nicht benötigt)
...
// da jedes I2C-Bit 2ms lang ist und ein I2C-Frame mit 3*9Bits=27*2ms=58ms dauert
// wenn mehr als ein I2C-Frame hier gesendet/empfangen werden soll
// pro Aufruf von io_update() nur ein I2C-Frame senden!
void io_update(void)
{
...
int main(void) 
{
    ...
    (void)term_string("'Var.Watch %e %spotlight i2c_data' zur Variablendarstellung\n\r",ASYNCSYNC_BLOCK);
	(void)term_string("'v.draw %e scope.buf0 scope.buf1' zur Oszilloskopdarstellung\n\r",ASYNCSYNC_BLOCK);
