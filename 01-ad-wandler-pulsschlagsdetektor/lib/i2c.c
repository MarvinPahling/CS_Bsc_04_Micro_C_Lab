...
#define I2C_TIMER_PWM3  0
#define I2C_TIMER_TC0   1
#define I2C_TIMER_MODE I2C_TIMER_PWM3
...
static const struct i2c_pin_pair i2c_pin[SENSOR_MAX] = {
  {1 << 23, 1 << 18},
  {1 << 28, 1 << 19},
  {1 << 29, 1 << 20},
  {1 << 30, 1 << 2}
...
// firmware which uses a fully driven clock interface. However doing so
...
struct i2c_port_struct {
  uint32_t scl_pin;
  uint32_t sda_pin;
  uint8_t  addr_int[5]; /* Device address plus internal address */
...
static struct i2c_port_struct i2c_port[SENSOR_MAX];
...
#if I2C_TIMER_MODE == I2C_TIMER_TC0
  uint32_t dummy = *AT91C_TC0_SR;
  (void) dummy; /* ergänzt DJ */  
#elif I2C_TIMER_MODE == I2C_TIMER_PWM3
  uint32_t dummy = *AT91C_PWMC_ISR;
  (void) dummy; /* ergänzt DJ */  
#endif	
...
void i2c_init(void)
{
  ...
  *AT91C_TC0_RC = (MCK/2)/(4 * I2C_BAUDRATE);
  *AT91C_TC0_IER = 0x10; /* Enable RC trigger interrupt */
  *AT91C_TC0_CCR = 1; /* Enable */

  aic_mask_off  (AT91C_ID_TC0);
//  aic_set_vector(AT91C_ID_TC0,AT91C_AIC_SRCTYPE_POSITIVE_EDGE | AIC_INT_LEVEL_NORMAL, i2c_timer_isr_C);
  aic_set_vector(AT91C_ID_TC0,AT91C_AIC_SRCTYPE_INT_HIGH_LEVEL | AIC_INT_LEVEL_ABOVE_NORMAL,i2c_timer_isr_C);
...
#elif I2C_TIMER_MODE == I2C_TIMER_PWM3
  /* Set up PWM */
  *AT91C_PMC_PCER = (1 << AT91C_ID_PWMC);    /* Power enable */
    
  *AT91C_PWMC_CH3_CMR = 0x1;                   // MCK/2 LeftAligned StartsAtLow
  *AT91C_PWMC_CH3_CPRDR = (MCK/2)/(4 * I2C_BAUDRATE);  //Period
  *AT91C_PWMC_CH3_CDTYR = (MCK/2)/(8 * I2C_BAUDRATE);  //DutyCycle
  
  dummy = *AT91C_PWMC_ISR;            //Clear Pending IRQ
  *AT91C_PWMC_IER = AT91C_PWMC_CHID3; //Enable PWM Channel 3 Interrupt
  
  aic_mask_off  (AT91C_ID_PWMC);
  aic_set_vector(AT91C_ID_PWMC,
                 AT91C_AIC_SRCTYPE_INT_HIGH_LEVEL | AIC_INT_LEVEL_ABOVE_LOW ,
				 i2c_timer_isr_C);
  
  aic_clear     (AT91C_ID_PWMC);
  aic_mask_on   (AT91C_ID_PWMC);
  *AT91C_PWMC_ENA = AT91C_PWMC_CHID3; //Enable PWM Channel 3
#endif
...
int i2c_wait_ready(sensor_t port, int32_t wait_ms)
{
	if(port < SENSOR_MAX) {
		...
		while((int)(systick_get_ms()-start) < wait_ms) {
			if (i2c_port[port].state == I2C_IDLE)
				return 0;
		}
	}
	return -1; /* time out */
...
{ 
  ...
  if(ninternal_address > 0){
    // Set up command to write the internal address to the device
    p->addr_int[0] = (address << 1); // This is a write
    p->addr_int[1] = internal_address[0];
	switch(ninternal_address) {
		case 4:
			p->addr_int[4] = internal_address[3]; __attribute__((fallthrough));
		case 3:
			p->addr_int[3] = internal_address[2]; __attribute__((fallthrough));
		case 2:
			p->addr_int[2] = internal_address[1];
	}
    ...
    pt->tx = 1;
    pt->data   = p->addr_int;
    pt->nbytes = ninternal_address+1;
    
    pt++;
  }

  if(ninternal_address == 0 || !write){  
    // Set up second partial transaction: restart and address
    pt->start = (ninternal_address > 0) ? 0 : 1;
    pt->restart = !pt->start;
    pt->stop = 0;
    pt->tx = 1;
...
{ uint8_t xx[20];
  int result;
  avr_set_sensor_power(SENSOR_1,SENSOR_9V);
  i2c_enable(0);
  while(1){
    systick_wait_ms(1000);
    i2c_start_transaction(0,1,0,1,xx,4,0,NULL,NULL);
    //i2c_start_transaction(0,1,0x42,1,xx,1,0,NULL,NULL);
    systick_wait_ms(1000);
    result = i2c_busy(0);
  }
}
