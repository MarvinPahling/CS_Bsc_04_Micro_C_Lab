...
#include "udmon3.h"
#include "../lib/aic.h"  //fuer aic_sys_vl_t aic_sys_register_pit()
...
struct {
	byte_fifo_t *term_send;
	byte_fifo_t *term_recv;
...
void udmon3_init(void) 
{
  ...
  aic_sys_register_pit(&udmon3_data.aic_sys_vl,udmon3_handler);
}

void udmon3_term_init(byte_fifo_t *recv,byte_fifo_t *send)
{
	udmon3_data.term_send=send;
	udmon3_data.term_recv=recv;
}
...
#define DCC_SendStatus()    ({int status; \
                              asm inline("mrc p14, 0, %0    , c0, c0" : "=r" (status));\
                              status & 2;})
...
static void Monitor_ReadByte (void * buf, void * address, int len)
{
  ...
  {
	if(udmon3_data.display_buffer == NULL)
		udmon3_data.display_buffer = display_get_buffer();
	...
	{ 
		...
		if((lcd_y >= 0) && (lcd_y < LCD_HIGH) && (lcd_x >= 0) && (lcd_x < LCD_WIDTH))
			target[i] = udmon3_data.display_buffer[((lcd_y/8)*LCD_WIDTH) + lcd_x] & (1<<(lcd_y%8)) ? 200 : 50;
		else
			target[i] =0;
	}
  }
...
static void Monitor_ReadHalf (void * buf, void * address, int len)
{
  ...
  if(((int)source & 0xffC00000) == LCD_MEMORY_OFFSET)
  {
	...
	for (i = 0; i < len; i++)
	{
		...
		int lcd_y = (((int)&source[i] - LCD_MEMORY_OFFSET) / NXT_WIDTH) - LCD_Y;
		if((lcd_y >= 0) && (lcd_y < LCD_HIGH) && (lcd_x >= 0) && (lcd_x < LCD_WIDTH))
			target[i] = (target[i]&0xff00) | ((udmon3_data.display_buffer[((lcd_y/8)*LCD_WIDTH) + lcd_x] & (1<<(lcd_y%8)) ? 200 : 50) << 0);
		else
			target[i] = (target[i]&0xff00) | (0                                                                           << 0);
		lcd_x++;
		if((lcd_y >= 0) && (lcd_y < LCD_HIGH) && (lcd_x >= 0) && (lcd_x < LCD_WIDTH))
			target[i] = (target[i]&0x00ff) | ((udmon3_data.display_buffer[((lcd_y/8)*LCD_WIDTH) + lcd_x] & (1<<(lcd_y%8)) ? 200 : 50) << 8);
		else
...
static void Monitor_ReadWord (void * buf, void * address, int len)
{
  ...
  if(((int)source & 0xffC00000) == LCD_MEMORY_OFFSET)
  {
	...
	for (i = 0; i < len; i++)
	{
		...
		int lcd_y = (((int)&source[i] - LCD_MEMORY_OFFSET) / NXT_WIDTH) - LCD_Y;
		if((lcd_y >= 0) && (lcd_y < LCD_HIGH) && (lcd_x >= 0) && (lcd_x < LCD_WIDTH))
			target[i] = (target[i]&0xffffff00) | ((udmon3_data.display_buffer[((lcd_y/8)*LCD_WIDTH) + lcd_x] & (1<<(lcd_y%8)) ? 200 : 50) << 0);
		else
			target[i] = (target[i]&0xffffff00) | (0                                                                           << 0);
		lcd_x++;
		if((lcd_y >= 0) && (lcd_y < LCD_HIGH) && (lcd_x >= 0) && (lcd_x < LCD_WIDTH))
			target[i] = (target[i]&0xffff00ff) | ((udmon3_data.display_buffer[((lcd_y/8)*LCD_WIDTH) + lcd_x] & (1<<(lcd_y%8)) ? 200 : 50) << 8);
		else
			target[i] = (target[i]&0xffff00ff) | (0                                                                           << 8);
		lcd_x++;
		if((lcd_y >= 0) && (lcd_y < LCD_HIGH) && (lcd_x >= 0) && (lcd_x < LCD_WIDTH))
			target[i] = (target[i]&0xff00ffff) | ((udmon3_data.display_buffer[((lcd_y/8)*LCD_WIDTH) + lcd_x] & (1<<(lcd_y%8)) ? 200 : 50) << 16);
		else
			target[i] = (target[i]&0xff00ffff) | (0                                                                           << 16);
		lcd_x++;
		if((lcd_y >= 0) && (lcd_y < LCD_HIGH) && (lcd_x >= 0) && (lcd_x < LCD_WIDTH))
			target[i] = (target[i]&0x00ffffff) | ((udmon3_data.display_buffer[((lcd_y/8)*LCD_WIDTH) + lcd_x] & (1<<(lcd_y%8)) ? 200 : 50) << 24);
		else
...
void udmon3_handler(void)
{
  ...
  // Der Debugger benötigt ebenfalls Zeit zum Bedienen des DCC-Channels
  // so dass eine for-Schleife hier keine Beschleunigung bringt
  if (DCC_ReceiveStatus())
  {
    ...
    {
      case 0x00:
	    if(udmon3_data.term_recv) {
          switch (data >> 24) {
            case 0x00: // reserved for terminal input in DCC3 protocol mode
			  byte_fifo_put(udmon3_data.term_recv,(unsigned char)((data>> 0)&0xff));
			  break;
            case 0x01:
			  byte_fifo_put(udmon3_data.term_recv,(unsigned char)((data>> 0)&0xff));
			  byte_fifo_put(udmon3_data.term_recv,(unsigned char)((data>> 8)&0xff));
			  break;
            case 0x02:
			  byte_fifo_put(udmon3_data.term_recv,(unsigned char)((data>> 0)&0xff));
			  byte_fifo_put(udmon3_data.term_recv,(unsigned char)((data>> 8)&0xff));
			  byte_fifo_put(udmon3_data.term_recv,(unsigned char)((data>>16)&0xff));
			  break;
            case 0x04:
            case 0x05:
            case 0x06:
...
#endif
	if(udmon3_data.term_send) {
	    if(byte_fifo_get(udmon3_data.term_send,((unsigned char *)&data)+0) != -1) {
		    if(byte_fifo_get(udmon3_data.term_send, ((unsigned char *)&data)+1) != -1) {
			    if(byte_fifo_get(udmon3_data.term_send, ((unsigned char *)&data)+2) != -1) {
