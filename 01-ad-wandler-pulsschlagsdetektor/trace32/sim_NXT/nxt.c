...
static void display_char(simulProcessor processor,int zei,int links, int unten, int bc, int fc)
{
	simulWord data;
	simulWord address;
	simulWord x;
...
static void display_printf(simulProcessor processor,int links, int unten, int bc, int fc,const char *format, ...)
{
	...
	for(lauf=0; string[lauf]!=0; lauf++)
		display_char(processor,(int)string[lauf],links+5*lauf,unten,bc,fc);
}

/**************************************************************************/

static void avr_init(simulProcessor processor,simulPtr private)
{
                        ...
                        simulWord dadr, simulWord iadr, simulWord byte, simulWord *rhr)
{
	...
	{
		SIMUL_Printf(processor,"NXT Nicht unterstützte TWI-Receive-DeviceAdresse SOLL=1 IST=%d",dadr);
		return;
	}
                     ...
                     simulWord dadr, simulWord iadr, simulWord byte, simulWord thr)
{
	...
	if(dadr != NXT_AVR_ADDRESS)
	{
		SIMUL_Printf(processor,"NXT Nicht unterstützte TWI-Transmit-DeviceAdresse SOLL=1 IST=%d",dadr);
		return;
	}
...
static void lcd_init(simulProcessor processor,simulPtr private)
{
