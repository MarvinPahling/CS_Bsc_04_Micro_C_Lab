...
//	adc_channel_set(ADC_CHANNEL_VMBT);
	...
	if(trig==ADC_TRIG_SYSTICK)
		aic_sys_register_pit(&adc_data.adc_aic_sys_vl,adc_channel_convert);
}
...
uint16_t adc_get_usb_mv(void)
{
	// Uq   = 3.3V/1023
	// Ulsb = Uq * (22k+68k) / 22k = 13,196mV
	// Uadc = 9v * 22k / (68k+22k)
	// UBAT = DIG * Ulsb
	// 0,013196V/DIG = 13,196mV/DIG  wird angenähert durch   
	// 13,196   wird angenähert durch 13513/1024
	return ((AT91C_ADC_CDR0[ADC_CHANNEL_USB] & 0x3ff) * 13513) >> 10;
}
